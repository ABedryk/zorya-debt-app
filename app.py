...
st.title("🔍 Перевірка боргу з податку за землю та податку на нерухоме майно на 01.05.2026")
st.write("Для платників, які мають майно на території Зорянської сільської територіальної громади")
            ...
            for index, row in results.iterrows():
                with st.container():
                    ...
                    with st.expander("💳 Показати реквізити для оплати"):
                        st.code(f"""Отримувач: ГУК у Рівн.обл/Зорянс. сіл.тг/
Рахунок (IBAN): {row['Бюджетний рахунок']}
МФО: {MFO}
ЄДРПОУ: {EDRPOU}
Призначення: Оплата заборгованості ({row['Назва коду класифікації доходів бюджету']}) від {row['ПІБ']}""", language="text")
                    st.divider()
        else:
